...
void solve() {
  read(N, M);
  edges.resize(M + 1);
  grph.assign(N + 1, V<int>(N + 1, INF));
  for (int i = 1; i <= M; ++i) read(edges[i].u, edges[i].v, edges[i].t);
  for (int i = 1; i <= N; ++i) {
    grph[i][i] = 0;
  }
  for (const auto& [u, v, t] : edges) {
    grph[u][v] = min(grph[u][v], t), grph[v][u] = min(grph[v][u], t);
  }
  for (int k = 1; k <= N; ++k) {
    for (int i = 1; i <= N; ++i) {
      for (int j = 1; j <= N; ++j) if(grph[i][k] != INF && grph[k][j] != INF) {
  ...
  rep(Q) {
    ...
    do {
      for (int mask = 0; mask < (1 << K); ++mask) {
        int can = 0, last = 1;
        for (int i = 0; i < K; ++i) {
          const auto [u, v, t] = edges[B[i]];
          if (mask & (1 << i)) { // u -> v
            can = can + grph[last][u] + t;
            last = v;
          } else { // v -> u
            can = can + grph[last][v] + t;
            last = u;
          }
        }
        can = can + grph[last][N];
