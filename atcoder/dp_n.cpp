...
signed main() {
    ...
    // minimim cost to combine each subarray
    V<V<int>> C(n+1, V<int>(n+1, INF));

    // also need the weight of each subarray, using prefix sum
    V<int> _W(a.begin(), a.end());
    for (int i = 1; i < (int)_W.size(); ++i) {
        _W[i] += _W[i-1];
    }
    ...
    // the order of iterating is important
    // need to iterate in reverse row-major order
    for (int l = n; l >= 1; --l) {
        for (int r = l; r <= n; ++r) {
            if (l == r) {
                C[l][r] = 0;
                ...
                for (int g = l; g <= r; ++g) {
                    if (g == l) {
                        C[l][r] = min(C[l][r], W(l, l) + W(l+1, r) + C[l][l] + C[l+1][r]);
                    } else if (g == r) {
                        C[l][r] = min(C[l][r], W(r, r) + W(l, r-1) + C[r][r] + C[l][r-1]);
                    } else {
                        C[l][r] = min(
                            C[l][r],
                            min(W(l, g) + W(g+1, r) + C[l][g] + C[g+1][r], 
                                W(l, g-1) + W(g, r) + C[l][g-1] + C[g][r])
                        );
                    }
                }
            }
