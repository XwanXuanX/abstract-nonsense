...
void dfs(int u, int& V, int& E) {
  ...
  E += grph[u].size();
  for (int v : grph[u]) {
    if (!vis.count(v)) dfs(v, V, E);
  }
}

void solve() {
  read(N);
  rab.resize(N), read(rab);
  for (const auto [a, b] : rab) {
    const int l = a - b, r = a + b;
    grph[l].push_back(r), grph[r].push_back(l);
  }
  int ans = 0;
  
  for (const auto& [n, _] : grph) {
