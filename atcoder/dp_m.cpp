...
signed main() {
    ...
    // if there are 0 candies, there is always one way to distribute
    for (int i = 0; i <= n; ++i) {
        dp[i][0] = 1;
    }
    // if there are more than 0 candies and no people, we cannot have leftovers
    for (int i = 1; i <= k; ++i) {
        dp[0][i] = 0;
    }

    for (int i = 1; i <= n; ++i) {
        // we need to get rid of one nested for loop, try prefix sum
        V<Mint> pfs(dp[i-1].begin(), dp[i-1].end());
        for (int i = 1; i < pfs.size(); ++i) {
            pfs[i] += pfs[i-1];
        }
        for (int j = 1; j <= k; ++j) {
            int rb = j, lb = j - min(j, a[i]);
            dp[i][j] = pfs[rb];
            if (lb > 0) {
                dp[i][j] -= pfs[lb-1];
            }
