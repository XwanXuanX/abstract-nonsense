...
signed main() {
    ...
    for (int i = 1; i <= n; ++i) {
        re(a[i].first, a[i].second);
    }

    // this problem is the same as the previous one, just the bounds are way larger
    // thus we need to redefine the dp states
    // technique used exactly the same as CF: Potions (C1)
    ...
    V<V<int>> dp(n+1, V<int>(sum+1, INF));
    // to achieve a value of 0, we need no weights
    for (int i = 0; i <= n; ++i) {
        dp[i][0] = 0;
    }
    ...
    for (int i = 1; i <= n; ++i) {
        for (int j = 1; j <= sum; ++j) {
            // still the problem of including or excluding
            dp[i][j] = dp[i - 1][j];
            if (j - a[i].second < 0) {
                continue;
            }
