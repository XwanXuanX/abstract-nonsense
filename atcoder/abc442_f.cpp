...
void solve() {
  int n; read(n);
  V<string> g(n); read(g);
  V<int> prev(n + 1), cur(n + 1), suf(n + 2);
  for (int i = 0; i < n; i++) {
    V<int> prefB(n + 1, 0);
    int totB = 0;
    for (int j = 0; j < n; j++) {
      prefB[j + 1] = prefB[j] + (g[i][j] == '#');
      totB += (g[i][j] == '#');
    }
    for (int k = 0; k <= n; k++) {
      int leftB = prefB[k];
      int rightW = (n - k) - (totB - prefB[k]);
      int cost = leftB + rightW;
      if (i == 0) prev[k] = cost;
      else cur[k] = cost + suf[k];
    }
    if (i > 0) prev.swap(cur);
    if (i + 1 < n) {
      suf[n] = prev[n];
      for (int k = n - 1; k >= 0; k--) suf[k] = min(prev[k], suf[k + 1]);
    }
  }
  int ans = prev[0];
  for (int k = 1; k <= n; k++) ans = min(ans, prev[k]);
  print(ans);
}
