...
/*
from k to k+1, the inversion number from [1, N-1] does not change
need to find # of a[i] (1<=i<=N-1) such that a[i] > a[0], and add to answer
0 3 1 5 4 2 9 6 8 7
3 1 5 4 2 9 6 8 7 0
1 5 4 2 9 6 8 7 0 3
*/
ll inversion(const V<int>& a) {
  int n = a.size();
  Fenwick fw(n);
  ll inv = 0;
  for (int i = n-1; i >= 0; --i) {
...
void solve() {
  int n; read(n);
  V<int> a(n); read(a);
  auto under = [&](const int p) -> int { return p; };
  auto above = [&](const int p) -> int { return n - 1 - p; };
  int inv = inversion(a);
  print(inv);
  for (int i = 0; i < n - 1; ++i) {
    inv -= under(a[i]);
