...
void solve() {
  read(N, M, S, T, Q);
  for (const auto c : S) s.insert(c);
  for (const auto c : T) t.insert(c);
  rep(Q) {
    string w; read(w);
    int tak = 1, aok = 1;
    for (const auto c : w) if (!s.count(c)) tak = 0;
    for (const auto c : w) if (!t.count(c)) aok = 0;
    if (tak && !aok) print("Takahashi");
    else if (!tak && aok) print("Aoki");
    else print("Unknown");
  }
}
