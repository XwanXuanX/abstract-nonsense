...
signed main() {
    ...
    // each dp state represents the LCS of s[1...i] and t[1...j]
    // also we need to remember which state did we transition from for later recovery
    V<V<int>> dp(n+1, V<int>(m+1, 0));
    V<V<int>> trans(n+1, V<int>(m+1));

    for (int i = 1; i <= n; ++i) {
        for (int j = 1; j <= m; ++j) {
            if (s[i-1] == t[j-1]) {
                int d = dp[i-1][j-1] + 1, l = dp[i][j-1], u = dp[i-1][j];
                int mx = max({d, l, u});
                dp[i][j] = mx;
                if (mx == d) {
                    trans[i][j] = D;
    ...
    // now we need to recover the paths to reconstruct the longest string
    string ans;
    int i = n, j = m;
    while (i >= 1 && j >= 1) {
        int tr = trans[i][j];
