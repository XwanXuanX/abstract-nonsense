...
signed main() {
    ...
    V<V<V<double>>> dp(n+1, V<V<double>>(n+1, V<double>(n+1, -1)));

    function<double(int i, int j, int k)> dfs;
    dfs = [&](int i, int j, int k) -> double {
        if (i < 0 || j < 0 || k < 0) {
            return 0;
        }
        if (i == 0 && j == 0 && k == 0) {
        ...
        double res = n + i*dfs(i-1, j, k) + j*dfs(i+1, j-1, k) + k*dfs(i, j+1, k-1);
    ...
    cout << fixed << setprecision(15) << dfs(cnt[1], cnt[2], cnt[3]) << '\n';
}