...
signed main() {
    ...
    // this is another inclusion and exclusion problem
    // the dp technique used is very similar to knapsack with large W

    V<V<double>> dp(n+1, V<double>(n+1, 0));
    ...
    for (int i = 2; i <= n; ++i) {
        for (int j = 0; j <= i; ++j) {
            // either current is head, or it's tail
