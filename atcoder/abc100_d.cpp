...
max(|X| + |Y| + |Z|) = max(sx*X + sy*Y + sz*Z) for all (sx, sy, sz)
why? because the max is achieved when each sign matches the sign of the corresponding sum
just need to enumerate all 8 sign cases, another enumeration problem
*/
void solve() {
  int n, m; read(n, m);
  V<Cake> a(n);
  int ans = 0;
  for (int i = 0; i < n; ++i) read(a[i].x, a[i].y, a[i].z);
  for (const int sx : {1, -1}) {
    for (const int sy : {1, -1}) {
      for (const int sz : {1, -1}) {
        V<int> sum;
        for (int i = 0; i < n; ++i) {
          const int tot = sx*a[i].x + sy*a[i].y + sz*a[i].z;
          sum.push_back(tot);
        }
