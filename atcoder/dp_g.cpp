...
signed main() {
    ...
    // since G does not contain directed cycles, means G is a DAG
    // and for a DAG we can always iterate in the top sort order
    // to make sure that all previous states have been visited before visiting the next state
    ...
    // with the top sort order, let's run the dp on it!
    // since we don't have the transpose graph, it's easier to just run push dp instead of pull dp
