...
void solve() {
  ...
  rep(Q) {
    int c, x; read(c, x);
    auto it = sts.upper_bound(x); it--;
    const int start = *it, end = ends[start];
    assert(start <= x && x <= end);
    if (c == 1) {
      sts.erase(start), ends.erase(start);
      if (start <= x - 1) sts.insert(start), ends[start] = x - 1;
      if (x + 1 <= end) sts.insert(x + 1), ends[x + 1] = end;
    } else {
      print(end - start + 2);
    }
  }
