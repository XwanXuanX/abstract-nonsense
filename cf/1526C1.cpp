...
signed main() {
    ...
    // if we take 0 potions, our health would be 0 anyways
    for (int i = 0; i <= n; ++i) {
        dp[i][0] = 0;
    }
    // if we have one potion, and we take 1 potion
    dp[1][1] = a[1];

    for (int i = 2; i <= n; ++i) {
        for (int j = 1; j <= i; ++j) {
            // disable transition from invalid state
            if (dp[i - 1][j] >= 0) {
            ...
            // Why? Because our health cannot be negative, thus states with negative health should be
            // disabled for transition!
