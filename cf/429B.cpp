...
void solve() {
    int n, m; re(n, m);
    V<V<int>> mat(n+2, V<int>(m+2));
    for (int i = 1; i <= n; ++i) {
        for (int j = 1; j <= m; ++j) {
            re(mat[i][j]);
        }
    }
    V<V<int>> dp1(n+2, V<int>(m+2)), dp2(n+2, V<int>(m+2)),
              dp3(n+2, V<int>(m+2)), dp4(n+2, V<int>(m+2));
    for (int i = 1; i <= n; ++i) {
        for (int j = 1; j <= m; ++j) {
    ...
    for (int i = 2; i < n; ++i) {
        for (int j = 2; j < m; ++j) {
            int a = dp1[i][j-1] + dp2[i][j+1] + dp3[i+1][j] + dp4[i-1][j];
            int c = dp1[i-1][j] + dp2[i+1][j] + dp3[i][j-1] + dp4[i][j+1];
            ans = max(ans, max(a, c));
            // ans = max(ans, dp1[i][j] + dp2[i][j] + dp3[i][j] + dp4[i][j] - (4 * mat[i][j]));
        }
    }
    ps(ans);
}
