...
void solve() {
  ...
  V<int> af, as;
  set<int> usedf, useds;
  // get elements that appear twice on each side
  for (int x : f) {
    if (af.size() >= 2 * k) {
      break;
    }
    if (!s.contains(x)) {
      af.push_back(x), af.push_back(x);
      usedf.insert(x);
    }
  }
  for (int x : s) {
    ...
    if (!f.contains(x)) {
      as.push_back(x), as.push_back(x);
      useds.insert(x);
    }
  }
  set_difference(all(f), all(usedf), inserter(f, f.begin()));
  set_difference(all(s), all(useds), inserter(s, s.begin()));
  for (int x : f) {
    if (af.size() == 2 * k) {
      break;
    }
    if (s.contains(x)) {
      af.push_back(x), as.push_back(x);
    }
  }
  print(af);
  print(as);
