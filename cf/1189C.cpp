...
void solve() {
    ...
    V<V<pair<int,int>>> dp(20);
    for (int i = 0; i < n; ++i) {
        dp[0].push_back(make_pair(a[i], 0));
    }
    int len = 1;
    for (int deg = 1; deg < 20; ++deg) {
        len *= 2;
        for (int i = 0; i + len <= n; ++i) {
            int left1 = dp[deg-1][i].first, left2 = dp[deg-1][i+len/2].first;
            int can1 = dp[deg-1][i].second, can2 = dp[deg-1][i+len/2].second;
            int left = (left1 + left2) % 10;
            int can = can1 + can2;
            if (left1 + left2 >= 10) {
                can++;
            }
            dp[deg].push_back(make_pair(left, can));
        }
    }
    re(q);
    for (int i = 0; i < q; ++i) {
