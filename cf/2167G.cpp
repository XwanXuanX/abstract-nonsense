...
void solve() {
  int n; read(n);
  V<int> a(n), c(n); read(a, c);
  set<int> unq(a.begin(), a.end());
  int id = 0;
  unordered_map<int, int> ID;
  for (const auto x : unq) {
    assert(!ID.count(x));
    ID[x] = id++;
  }
  for (int i = 0; i < n; ++i) {
  ...
  V<int> dp(id, LLONG_MAX);
  for (int j = 0; j < id; ++j) {
    dp[j] = a[0] == j ? 0 : c[0];
  }
  for (int i = 1; i < n; ++i) {
    V<int> MN(id, LLONG_MAX);
    MN[0] = dp[0];
    for (int j = 1; j < id; ++j) {
      MN[j] = min({MN[j], MN[j-1], dp[j]});
    }
    for (int j = 0; j < id; ++j) {
      const int mn = MN[j];
      const int self_cost = a[i] == j ? 0 : c[i];
      dp[j] = min(LLONG_MAX, self_cost + mn);
    }
  }
  int ans = LLONG_MAX;
  for (int j = 0; j < id; ++j) {
