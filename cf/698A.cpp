...
void solve() {
    ...
    // dp[i][2] = the number of days to work if go to gym or go to study on day i
    V<V<int>> dp(n+1, V<int>(2, 0));
    for (int i = 1; i <= n; ++i) {
        if (a[i] == 0) {
            // we can't do anything
            dp[i][C] = dp[i][G] = max(dp[i-1][C], dp[i-1][G]);
        } else if (a[i] == 1) {
            // we can do contest, or do nothing
            int nothing = max(dp[i-1][C], dp[i-1][G]);
            dp[i][C] = max(nothing, dp[i-1][G] + 1);
            dp[i][G] = nothing;
        } else if (a[i] == 2) {
            // we can do gym, or do nothing
            int nothing = max(dp[i-1][C], dp[i-1][G]);
            dp[i][G] = max(nothing, dp[i-1][C] + 1);
            dp[i][C] = nothing;
        } else {
            // we can do both, or do nothing
            int nothing = max(dp[i-1][C], dp[i-1][G]);
            dp[i][G] = max(nothing, dp[i-1][C] + 1);
            dp[i][C] = max(nothing, dp[i-1][G] + 1);
        }
