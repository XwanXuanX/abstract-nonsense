...
void solve() {
    ...
    // find the number of repeated a needed to achieve at least x
    int least = cdiv(x, tot);
    if (least > k) {
        print(0);
        return;
    }
    // otherwise, there must be an answer
    int rem = x - (least - 1) * tot;
    int acc = 0;
    int i = n - 1;
    for (i = n - 1; i >= 0; --i) {
        acc += a[i];
...
/**
 * obs:
 * if the answer exists, there must exists a point which every point including it is valid,
 * and all points to the right of it are invalid. we need to find such point.
 * but we have to check if the answer exists or not.
 * can also use binary search, also rely on the fact of monotonicity.
 */
