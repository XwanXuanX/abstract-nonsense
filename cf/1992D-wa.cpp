...
void solve() {
    ...
    dp[0] = true;
    for (int i = 0; i <= n; ++i) {
        // if we cannot land in this cell, do nothing
        if (!dp[i]) {
            continue;
        }
        // if we are on bank or log, try jump
        if (i == 0 || s[i] == 'L') {
            for (int j = 1; j <= m; ++j) {
                if (s[i+j] == 'C') {
                    continue;
            ...
            // if we are in water, try swim
            assert(s[i] == 'W');
            // if the water has already been reached before
            // decide if we need to swim to next one
            if (dp[i] && !dp[i+1]) {
                if (swimmed < k) {
                    for (int j = 1; j <= k; ++j) {
                        // we cannot run into a crocodile
                        if (s[i+j] == 'C') {
                            break;
                        }
                        dp[i+j] = true;
                        ++swimmed;
                        // if we've run into a surface, then quit swim
                        if (s[i+j] == 'L' || i + j == n + 1) {
                            break;
                        }
                    }
