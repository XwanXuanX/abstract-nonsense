...
void solve() {
    ...
    assert(fsi != -1);
    if (n % 2 != 0) {
        print((fsi <= cdiv(n, 2)) ? "YES" : "NO");
    } else {
        print((fsi <= cdiv(n, 2) + 1) ? "YES" : "NO");
    }
}

/**
 * obs:
 * the idea is, after sorting the numbers in increasing order,
 * if the first element is at the first half of the array, then we can always make some element
 * in the second half negative, so that it's the medium.
 * but if it's in the second half of the array, then it's not possible
 */
