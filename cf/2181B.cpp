...
void solve() {
  int n, m; read(n, m);
  V<int> a(n), b(m); read(a, b);
  MxPQ<int> pqa, pqb;
  for (const auto x : a) pqa.push(x);
  for (const auto x : b) pqb.push(x);
  int turn = 0;
  while (pqa.size() && pqb.size()) {
    if (turn == 0) { // alice
      const int ap = pqa.top(), bp = pqb.top(); pqb.pop();
      if (ap < bp) pqb.push(bp - ap);
      if (pqb.size() == 0) {
        print("Alice"); return;
      }
      turn = 1;
    } else { // bob
      const int bp = pqb.top(), ap = pqa.top(); pqa.pop();
      if (bp < ap) pqa.push(ap - bp);
      if (pqa.size() == 0) {
        print("Bob"); return;
      }
      turn = 0;
