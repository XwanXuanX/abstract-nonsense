...
void solve() {
    ...
    // if after the first move, Tom cannot make mx - mn <= k
    // then Tom lose directly, and Jerry 
    if (mx - 1 - mn > k || (mx - mn > k && mxcnt > 1)) {
        ps("Jerry");
        return;
    }
    // what should Jerry do in the next round?
    // Jerry cannot decrease mn, since that would cause him to lose directly
    // as long as after the first move mx - mn <= k, both plays will try to avoid mx - mn > k
    // cuz in that case the last mover will lose
    int tot = accumulate(a.begin(), a.end(), 0);
    ps(tot % 2 == 0 ? "Jerry" : "Tom");
}
