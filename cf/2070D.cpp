...
void solve() {
    ...
    while(que.size()) {
        ...
        vector<int> lvl;
        for(int i=0; i<sz; ++i) {
            auto[nd,par]=que.front();
            que.pop();
            lvl.push_back(nd);
            for(auto&& adj:T[nd]) if(adj!=par) {
                que.push({adj,nd});
            }
        }
        cat.push_back(std::move(lvl));
    }
    // for(auto&& lvl:cat) {
        // for(auto&& nd:lvl) cout<<nd<<' '; cout<<'\n';
    // }
    vector<Mint> dp(n+1,1);
    // for(auto&& nd:cat.back()) {
        // dp[nd]=1;    
    // }
    for(int i=cat.size()-2; i>=1; --i) {
        Mint all=0;
        for(auto&& adj:cat[i+1]) {
            all+=dp[adj];
        }
        for(auto&& nd:cat[i]) {
            Mint sub=0;
