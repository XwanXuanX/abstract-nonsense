...
void solve() {
    ...
    constexpr int prime = 1e9 + 7; // we all know it's prime LOL
    int mni = min_element(a.begin(), a.end()) - a.begin();
    for (int i = 0; i < n; ++i) {
        if (i == mni) {
            continue;
        }
        if (mni % 2) { // at odd, then make even prime
            if (i % 2) {
                ans.push_back(make_tuple(mni+1, i+1, a[mni], a[mni]));
            } else {
                ans.push_back(make_tuple(mni+1, i+1, a[mni], prime));
            }
        } else { // at even, make odd prime
            if (i % 2) {
                ans.push_back(make_tuple(mni+1, i+1, a[mni], prime));
            } else {
                ans.push_back(make_tuple(mni+1, i+1, a[mni], a[mni]));
...
/**
 * what the operation said, is that: for any two number a and b (where a < b)
 * you can select a number x >= a, and assign it to b (if contains 1 then all good)
 * note that you cannot go below the min(a1, a2, ... , an)
 */
