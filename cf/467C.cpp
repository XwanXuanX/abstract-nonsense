...
void solve() {
    ...
    // dp[i][j] = for the prefix [1...i], if there are j pairs in total,
    // what is the maximum sum
    // the recurrence:
    // dp[i][j] = max(dp[i-1][j], dp[i-m][j-1] + range_sump[i-m...i])
    V<V<int>> dp(n+1, V<int>(k+1, 0));
    for (int i = 1; i <= n; ++i) {
        for (int j = 0; j <= k; ++j) {
            // if we don't pick a[i] as the right bound of a pair
            dp[i][j] = dp[i-1][j];
            // if we pick a[i] as the right bound
            if (i - m >= 0 && j >= 1) {
