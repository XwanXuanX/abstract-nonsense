...
void solve() {
    ...
    // basically we just need to find the longest subarray with sum s
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (psum == s) {
            mx = i + 1;
        } else if (mp.count(psum - s)) {
            mx = max(mx, i - mp[psum - s]);
        }
        // only store the first occurance of the prefix
        if (!mp.count(psum)) {
            mp[psum] = i;
        }
    }
