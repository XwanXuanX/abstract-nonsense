...
void solve() {
    ...
    int ans = 1;
    int start = 0;
    while (start < b.size() - 1) {
        int end = start;
        if (b[end + 1] > b[end]) {
            while (end + 1 < b.size() && b[end + 1] > b[end]) {
                end++;
            }
            ans++;
        } else {
...
/**
 * for any increasing or decreasing sequence, we only need to keep the end point
 * in that case, we can definitely make the contrast equal
 */
