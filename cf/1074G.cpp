...
void solve() {
  ...
  V<int> MEX(n, 0), NEXT(n, 0);
  V<unordered_map<int, int>> OKR(n);
  for (int i = 0; i < n; ++i) {
    const auto& ai = a[i];
    for (const auto x : ai) {
      OKR[i][x]++;
    ...
    int mex = 0;
    while (OKR[i][mex] > 0) mex++;
    MEX[i] = mex;
    int nxt = mex + 1;
    while (OKR[i][nxt] > 0) nxt++;
    NEXT[i] = nxt;
  }
  int tot = 0;
  unordered_map<int, int> MEX_count, SUM_next;
  for (int i = 0; i < n; ++i) {
    tot += MEX[i];
    MEX_count[MEX[i]]++;
    SUM_next[MEX[i]] += NEXT[i];
  }
  int ans = 0;
  for (int i = 0; i < n; ++i) {
    for (int j = 0; j < a[i].size(); ++j) {
      const int b = a[i][j];
      // since b is in a[i], then MEX[i] != b
      assert(MEX[i] != b);
      const int minus = MEX_count[b] * b, plus = SUM_next[b];
      int acc = (tot - MEX[i]) * (n - 1) - minus + plus;
      if (b > MEX[i]) {
        acc += MEX[i] * (n - 1);
      } else {
        if (OKR[i][b] == 1) {
