...
void solve() {
    ...
    V<int> dp(n+1, 0);
    for (int i = 1; i <= n; ++i) {
        // for each of the element, we can either choose it or not
        dp[i] = dp[i-1];
        if (best.count(a[i])) {
            auto b = best[a[i]];
            dp[i] = max(dp[i], b.first + (i - b.second + 1));
        }
        if (!best.count(a[i]) || dp[i-1] - i > best[a[i]].first - best[a[i]].second) {
            best[a[i]].first = dp[i-1];
            best[a[i]].second = i;
        }
    }
