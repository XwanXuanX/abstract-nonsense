...
void solve() {
    ...
    // if no 0 or no 1, then just output
    if (l0 == -1 || f1 == -1 || l0 < f1) {
        ps(s);
        return;
    }
    string ans;
    // get the prefix zeros
    for (int i = 0; i < f1; ++i) {
        ans.push_back(s[i]);
    }
    // in the middle, just put 1 0
    ans.push_back('0');
    // get the sufix ones
...
/**
 * we can see that if there is at least 1 0 after 1s, then all the 1s can be removed
 * then we'll eventually get a string like 00000....000000111111....111111
 * now we need to make the string just shorter
 * 11001101
 * 1100101 - 110001 - 11001 - 1101 - 101 - 01 - done
 */
