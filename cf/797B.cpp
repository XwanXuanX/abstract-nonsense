...
void solve() {
    ...
    for (int i = 1; i <= n; ++i) {
        if (abs(a[i]) % 2 == 0) {
            dp[i][E] = max(dp[i][E], max(a[i], best[E] + a[i]));
            if (best[O] != -INF) {
                dp[i][O] = max(dp[i][O], best[O] + a[i]);
            }
        } else {
            dp[i][O] = max(dp[i][O], max(a[i], best[E] + a[i]));
            if (best[O] != -INF) {
                dp[i][E] = max(dp[i][E], best[O] + a[i]);
            }
        }
        best[E] = max(best[E], dp[i][E]);
        best[O] = max(best[O], dp[i][O]);
    }
    ps(best[O]);
}
