...
bool ok(int w, int l, int h) {
  // check if the w l h can fit the first two boxes
  int f = fib[n], s = fib[n-1];
  int lside = f + s;
  // we need at least one side to fit lside, and the other two to fit f
  V<int> sides{w, l, h};
  sort(sides.begin(), sides.end());
  return sides[2] >= lside && sides[1] >= f && sides[0] >= f;
}

void solve() {
  read(n, m);
  box.assign(m, V<int>(3));
  for (int i = 0; i < m; ++i) {
    read(box[i][0], box[i][1], box[i][2]);
  }
  string ans;
  for (int i = 0; i < m; ++i) {
    if (ok(box[i][0], box[i][1], box[i][2])) {
      ans.push_back('1');
    } else {
      ans.push_back('0');
    }
